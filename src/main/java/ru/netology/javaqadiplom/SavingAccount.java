...
public class SavingAccount extends Account {
    ...
    public SavingAccount(int initialBalance, int minBalance, int maxBalance, int rate) {
        ...
        // добавлено исключение равенства максимального баланса нулю, т.к. это будет пустой счет, с которым ничего нельзя будет сделать
        if (maxBalance == 0) {
            throw new IllegalArgumentException(
                    "Максимальный баланс не может равняться нулю"
            );
        }
        ...
        if (initialBalance > maxBalance) {
            throw new IllegalArgumentException(
                    "Начальный баланс не может быть больше максисмального"
            );
        }
